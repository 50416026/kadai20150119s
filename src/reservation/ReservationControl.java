...
//　施設概要
	public String makeExplanation(MainFrame frame){
		String res="";
		
		return res = "小ホール\n"
				+ "30人収容できる。折り畳みのいすや机もあります。\n"
				+ "大会議室\n"
				+ "20人収容できる。机といすの位置は定位置に戻してください。給湯室付き。\n"
				+ "小会議室\n"
				+ "10人収容できる。グループでの会議にお勧め。\n";
	}
	
//　予約確認	
    public String makeConfirm(String facility, String ryear_str, String rmonth_str, String rday_str){
    	String res = "";
		// 年月日が数字かどうかををチェックする処理
		try {
			int ryear = Integer.parseInt( ryear_str);
    ...
    //キャンセル
    public String makeCancel(MainFrame frame){
		...
		if ( flagLogin){ // ログインしていた場合
			...
			// キャンセル画面のキャンセル日に，メイン画面に設定されている年月日を設定する
			rd.tfYear.setText(frame.tfYear.getText());
			rd.tfMonth.setText(frame.tfMonth.getText());
			rd.tfDay.setText(frame.tfDay.getText());
